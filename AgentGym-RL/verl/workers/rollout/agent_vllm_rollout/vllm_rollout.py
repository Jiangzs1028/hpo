...
def _find_subseq_positions(seq: List[int], pat: List[int]) -> List[int]:
    """Return all start indices i where seq[i:i+len(pat)] == pat."""
    if not pat or len(pat) > len(seq):
        return []
    out = []
    m = len(pat)
    # naive scan is fine for typical T
...
def split_steps(
    ...
    attention_mask: torch.Tensor,         # (B, T) 1=valid, 0=pad/invalid
    tokenizer,
    ...
    B, T = input_ids.shape
    step_index = torch.full((B, T), -1, dtype=torch.long, device=input_ids.device)

    a_pref_ids = tokenizer.encode(format_config["assistat_prefix_msg"], add_special_tokens=False)
    steps_texts: List[List[str]] = []
    ...
    for b in range(B):
        ...
        # 约束：有效 token 是连续区间（只允许左右 padding）
        if not torch.all(valid_pos[1:] == valid_pos[:-1] + 1):
            raise ValueError(f"attention_mask for sample {b} is not contiguous (has holes).")

        seg_valid_pos = valid_pos                      # 原始位置
        packed_ids = input_ids[b, seg_valid_pos].tolist()

        a_starts = sorted(set(_find_subseq_positions(packed_ids, a_pref_ids)))
        boundaries = [0] + a_starts + [len(packed_ids)]
        ...
        for i in range(len(boundaries) - 1):
            ...
            step_index[b, seg_valid_pos[s:e]] = step_id
            sample_steps.append(tokenizer.decode(packed_ids[s:e], skip_special_tokens=False))
            step_id += 1

        steps_texts.append(sample_steps)

    return steps_texts, step_index
...
class vLLMRollout(BaseRollout):
    ...
    def __init__(self, actor_module: nn.Module, rollout_config: DictConfig, agentgym_config: DictConfig, tokenizer, model_hf_config, **kwargs):
        ...
        max_num_batched_tokens = self.config.get('max_num_batched_tokens', rollout_config.response_length+rollout_config.prompt_length)
        ...
        self.inference_engine = LLM(
            ...
            max_model_len=rollout_config.response_length+rollout_config.prompt_length,
        )
        ...
        # if self.agentgym_config.task_name == 'searchqa':
        #     kwargs['stop'] = ["</search>", "</answer>"]
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        while rounds < max_rounds and not all_done_flag:
            ...
            if self.agentgym_config.task_name == "searchqa":
                ...
                trimmed_batch = []
                for seq in response_ids:  # seq: List[int]
                    decoded_so_far = ""
                    cut_idx = None
                    ...
                    for i, tid in enumerate(seq):
                        ...
                        # 找到第一个闭合 tag 就截断（包含该 tag 所在 token）
                        if any(tag in decoded_so_far for tag in end_tags):
                            cut_idx = i + 1
                            break

                    trimmed_batch.append(seq[:cut_idx] if cut_idx is not None else seq)

                response_ids = trimmed_batch
        ...
        if global_steps:
            try:
                step_dir = os.path.join(self.config.rollout_log_dir, f"step{global_steps}")
                os.makedirs(step_dir, exist_ok=True)

                jsonl_path = os.path.join(step_dir, f"{torch.distributed.get_rank()}.jsonl")
                ...
                with open(jsonl_path, "a", encoding="utf-8") as f:
                    for idx, msgs in enumerate(messages):
                        record = {
                            ...
                            "reward": float(scores[idx]) if hasattr(scores[idx], "item") else scores[idx],
                        }
                        f.write(json.dumps(record, ensure_ascii=False) + "\n")

            except Exception as e:
                print(f"[rollout jsonl dump error] {e}")
        ...
        steps_texts, step_index = split_steps(seq, attention_mask, self.tokenizer)
