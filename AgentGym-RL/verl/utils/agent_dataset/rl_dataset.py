...
def split_steps_text(
    conversations: List[Dict[str, str]],
    tokenizer,
    assistant_prefix: str = "\n<|im_start|>assistant\n",
) -> List[str]:
    """
    只做文本切分，不做任何 token/id 相关操作。

    规则：
      - 用 apply_chat_template 得到 full_text（与训练模板一致）
      - 以 assistant_prefix 作为切分锚点：
          step0 = [0 : first_assistant_prefix)
          stepk = [assistant_prefix_k : assistant_prefix_{k+1})  (k>=1)
      - 返回 steps_texts: List[str]
    """
    ...
    # 找所有 assistant_prefix 的起点
    starts = []
    i = 0
    while True:
        p = full_text.find(assistant_prefix, i)
        if p < 0:
            break
        starts.append(p)
        i = p + 1  # 安全起见，避免极端重叠问题
    ...
    boundaries = [0] + starts + [len(full_text)]

    steps_texts: List[str] = []
    for s, e in zip(boundaries[:-1], boundaries[1:]):
        if e > s:
            steps_texts.append(full_text[s:e])
...
class RLHFDataset(Dataset):
    ...
    def _build_messages(self, example: dict):
        ...
        messages = [{"role": "system", "content": self.env_client.conversation_start[0]["value"]},]
        prompt_with_chat_template = "<|im_start|>system\n"+self.env_client.conversation_start[0]["value"]+"<|im_end|>"
    ...
    def __getitem__(self, item):
        ...
        if self.encoder_config.enable_encoder == True and self.encoder_config.strategy == "off-policy":
            step_texts_off = []
            for traj in row_dict.get("success", []):
                conv = traj["conversations"]
                steps = split_steps_text(
                    conv, self.tokenizer, assistant_prefix=self.assistant_prefix
                )
                step_texts_off.append(steps)

            
            row_dict["step_texts_off"] = step_texts_off
