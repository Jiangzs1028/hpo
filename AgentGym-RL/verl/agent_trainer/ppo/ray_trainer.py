...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1,step_adv_coef=0):
    ...
    if adv_estimator == 'gae':
        ...
        data.batch['advantages'] = advantages
        data.batch['returns'] = returns
    elif adv_estimator == 'ot_grpo':
        step_level_advantages = data.batch["step_level_advantages"]
        token_level_rewards = data.batch['token_level_rewards']
        index = data.non_tensor_batch['uid']
        response_mask = data.batch['response_mask']
        advantages, returns = core_algos.compute_otgrpo_outcome_advantage(token_level_rewards=token_level_rewards,
                                                                        step_level_advantages=step_level_advantages,
                                                                        eos_mask=response_mask,
                                                                        index=index,
                                                                        step_adv_coef=step_adv_coef)
        data.batch['advantages'] = advantages
        data.batch['returns'] = returns
...
class RayPPOTrainer(object):
    ...
    def __init__(self,
        ...
        elif self.config.algorithm.adv_estimator == 'ot_grpo':
            self.use_critic = False
    ...
    def _create_dataloader(self):
        ...
        self.train_dataset = RLHFDataset(
            ...
            encoder_config=self.config.actor_rollout_ref.encoder
        )
    ...
    # def cosine_anneal_coef(self):
    #     if self.config.algorithm.adv_estimator != 'ot_grpo':
    #         return 0
    #     T = self.config.actor_rollout_ref.encoder.total_steps
    #     warmup_steps = self.config.actor_rollout_ref.encoder.warmup_steps
    #     step = min(self.global_steps, T)

    #     coef_max = self.config.actor_rollout_ref.encoder.step_adv_coef
    #     coef_min = self.config.actor_rollout_ref.encoder.step_adv_coef_min

    #     coef = coef_min + 0.5 * (coef_max - coef_min) * (
    #         1.0 + math.cos(math.pi * step / T)
    #     )
    #     return coef
    ...
    def cosine_anneal_coef(self):
        ...
        if warmup_steps > 0 and step < warmup_steps:
            return coef_max * (step / warmup_steps)

        # 余弦退火：coef_max -> coef_min
        # 注意：退火阶段的 step 需要减去 warmup_steps，并且周期长度也要减去 warmup_steps
        anneal_T = max(1, T - warmup_steps)
        anneal_step = min(step - warmup_steps, anneal_T)

        coef = coef_min + 0.5 * (coef_max - coef_min) * (
            1.0 + math.cos(math.pi * anneal_step / anneal_T)
        )
        return coef
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with _timer('step', timing_raw):
                    ...
                    with _timer("embedding", timing_raw):
                        reward_tensor = batch.batch['scores']
                        batch.batch['token_level_scores'] = reward_tensor
                        
                        if self.config.algorithm.adv_estimator == 'ot_grpo':
                            if self.config.actor_rollout_ref.encoder.strategy == "off-policy":
                                step_level_advantages = self.actor_rollout_wg.compute_step_advantages_off(batch)
                            elif self.config.actor_rollout_ref.encoder.strategy == "on-policy":
                                step_level_advantages = self.actor_rollout_wg.compute_step_advantages(batch)
                            else:
                                raise ValueError(f'Unsupported type in encoder.strategy {self.config.actor_rollout_ref.encoder.strategy}')
                            batch = batch.union(step_level_advantages)
                    ...
                    with _timer('adv', timing_raw):
                        ...
                        step_adv_coef = self.cosine_anneal_coef()
                        metrics.update({
                            'actor/step_adv_coef': step_adv_coef,
                        })
                ...
                if self.config.algorithm.adv_estimator == 'ot_grpo':
                    ...
                    output_ids = batch.batch["responses"] # (bs, seq_len)
                    output_texts = [self.tokenizer.decode(ids, skip_special_tokens=True) for ids in output_ids]
                    
                    scores = batch.batch["advantages"].cpu().tolist()

                    scores = [
                        [row[0]] + [row[i] for i in range(1, len(row)) if row[i] != row[i-1]]
                        if len(row) > 0 else []
                        for row in scores
                    ]
                    ...
                    samples = samples[: 8]
                    logger.log_sample(data=samples, step=self.global_steps)
